...
## take the % of SNVs generated by clock-like signatures 

clock.like <- clock.like.all
...
P.MRCA = data.frame(Density=mutation.time.mrca[tmm.tumors,]$Mean*clock.like[tmm.tumors])
...
P.MRCA$P.upper = sapply(P.MRCA$Density, function(x){
  sum(mutation.time.mrca[tmm.tumors,]$Min*clock.like[tmm.tumors] <= x, na.rm = T)
})/nrow(P.MRCA)
P.MRCA$P.lower = sapply(P.MRCA$Density, function(x){
  sum(mutation.time.mrca[tmm.tumors,]$Max*clock.like[tmm.tumors] <= x, na.rm = T)
})/nrow(P.MRCA)

## collect the cumulative distribution of ECA densities
P.ECA = data.frame(Density=mutation.time.eca[tmm.tumors,]$Mean*clock.like[tmm.tumors])
...
P.ECA$P.upper = sapply(P.ECA$Density, function(x){
  sum(mutation.time.eca[rownames(P.ECA),]$Min*clock.like[rownames(P.ECA)] <= x, na.rm = T)
})/nrow(P.ECA)
P.ECA$P.lower = sapply(P.ECA$Density, function(x){
  sum(mutation.time.eca[rownames(P.ECA),]$Max*clock.like[rownames(P.ECA)] <= x, na.rm = T)
})/nrow(P.ECA)


## collect the cumulative distribution of MRCA densities in low-risk tumors
P.MRCA.lr = data.frame(Density=mutation.time.mrca[no.tmm.tumors,]$Mean*clock.like[no.tmm.tumors])
...
P.MRCA.lr$P.upper = sapply(P.MRCA.lr$Density, function(x){
  sum(mutation.time.mrca[no.tmm.tumors,]$Min*clock.like[no.tmm.tumors] <= x, na.rm = T)
})/nrow(P.MRCA.lr)
P.MRCA.lr$P.lower = sapply(P.MRCA.lr$Density, function(x){
  sum(mutation.time.mrca[no.tmm.tumors,]$Max*clock.like[no.tmm.tumors] <= x, na.rm = T)
})/nrow(P.MRCA.lr)



save(P.MRCA, P.ECA, P.MRCA.lr, 
     file=paste0(rdata.directory, "Input_data_NB_initiation_clock_like.RData"))
