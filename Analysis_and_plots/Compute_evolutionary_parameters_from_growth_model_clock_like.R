...
### Neutral_fit_XTEN.py, executing Neutral_fit_pre_clonal_and_clonal.R
##############################################################################################################################################
...
clock.like <- clock.like.subclonal
...
for(i in rownames(subset)){
  ...
  fits$par_mu <- fits$par_mu*clock.like[i]
  ...
  mutational.burden.at.mrca <- mutation.time.mrca[i,]$Mean*clock.like[i]
...
save(estimated.mutation.rate.per.day, file=paste0(rdata.directory, "Estimated_mutation_rate_per_day_clock_like.RData"))
