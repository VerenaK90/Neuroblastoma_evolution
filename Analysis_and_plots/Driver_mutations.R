...
for(i in tumors.80x){
  ...
  ## read in the mutation file
  files <- list.files(paste0(data.directory.80x,  i, "/", indel.directory, "/"), pattern="somatic_indels_conf_8_to_10")[1]
  if(is.na(files)){
    print(i)
    next
  }
  
  mutations <- read.vcf(paste0(data.directory.80x,  i, "/", indel.directory, "/", files))
  ...
  ## read in the mutation file
  files <- list.files(paste0(data.directory.80x,  i, "/", sv.directory, "/"), pattern="filtered_somatic_minEventScore3.tsv")[1]
  if(is.na(files)){
    print(i)
    next
  }
  
  mutations <- read.vcf(paste0(data.directory.80x,  i, "/", sv.directory, "/", files))
  mutations <- mutations$vcf
...
tmp <- filtered.functional.mutations
...
setdiff(amplifications[amplifications$gene=="MYCN", "Sample"], names(telomere.classification.80x[telomere.classification.80x=="MNA"]))
setdiff( names(telomere.classification.80x[telomere.classification.80x=="MNA"]),amplifications[amplifications$gene=="MYCN", "Sample"])
...
mat <- matrix("", nrow=length(unique(driver.genes)), ncol=length(tumors.80x),
              dimnames=list(c(unique(driver.genes)), tumors.80x))
...
mat["ATRX",] <- sample.information.80x[colnames(mat), "ATRX"]
mat[is.na(mat)] <- ""
mat[mat=="wt"] <- ""
...
mat["MYCN",sample.information.80x[colnames(mat),"Telomere.maintenance.mechanism"]=="MNA"] <- paste(mat["MYCN",sample.information.80x[colnames(mat),"Telomere.maintenance.mechanism"]=="MNA"], "AMP", sep=";")
mat["TERT",sample.information.80x[colnames(mat),"Telomere.maintenance.mechanism"]=="TERT"] <- paste(mat["TERT",sample.information.80x[colnames(mat),"Telomere.maintenance.mechanism"]=="TERT"], "SV", sep=";")
## adjust heterogeneous cases (they were annnotated as "HET" in the excel file but have SVs in TERT and amplifications in MYCN)
mat["TERT","XI003_20992"] <- "SV"
mat["MYCN","XI003_20992"] <- "AMP"
mat["MYCN","XI003_NB-AUT-3"] <- "AMP"
mat["TERT","XI003_25026"] <- "SV"
