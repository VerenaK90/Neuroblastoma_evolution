...
drivers <- read.delim(paste0(meta.data, "Driver_mutations_cohort.tsv"))
...
## run in fast mode first and consider re-running if not satisfied
  ...
  ## plot the best model
  pdf(paste0(data.directory.80x, i, "/Mobster_fits.pdf"))
  ...
  save(fit, clusters, file=paste0(data.directory.80x, "/", i, "/Mobster_fits.RData"))
