...
## store figure panels
panel.directory <- paste0(output.directory, "Figure7/")
...
##############################################################################################################################################
## Figure 7a Expansion + homeostasis
...
pdf(paste0(panel.directory, "Figure_7a.pdf"), width=5, height=3)
...
addWorksheet(wb, "c")
writeData(wb, "c", to.store)


pdf(paste0(panel.directory, "Figure_7c.pdf"), width=5, height=3)
...
##############################################################################################################################################
## Figure 7d Expansion + decay, MRCA
...
pdf(paste0(panel.directory, "Figure_7d.pdf"), width=5, height=3)
print(ggplot(to.plot, aes(x=x, y = data, ymin = data-sd, ymax = data +sd, col=Event, fill=Event)) + geom_step() + geom_errorbar()+
        geom_stepribbon(aes(x=x, ymin = lower, ymax = upper), alpha=0.5)  +
...
##############################################################################################################################################
##### Figure 7e, S5b Plot the retraction of neuroblasts and the exapnsion of the first clone over approximate week of pregnancy
...
pdf(paste0(panel.directory, "Figure_7e.pdf"), width=7, height=7, useDingbats = F)
...
##############################################################################################################################################
## Figure 7f: effective mutation rate
...
saveWorkbook(wb, file = paste0(panel.directory, "Source_data_Fig.7.xlsx"), overwrite=T)
