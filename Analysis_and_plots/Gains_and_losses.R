...
for(i in tumors.discovery){
  ...
  aceseq <-  read.delim(paste0(data.directory.discovery,  i, "/", cnv.directory, "/", aceseq),  sep="\t", stringsAsFactors = F)
...
setdiff(tumors.discovery[telomere.classification.discovery[tumors.discovery]!="None"], unique(gains$tumor))
setdiff(names(telomere.classification.discovery[telomere.classification.discovery!="None"]), unique(losses$tumor))
