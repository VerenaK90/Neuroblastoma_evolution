...
- [Description of the R package NBevolution](#Description-of-the-R-package-NBevolution)
...
The folder [Analysis_and_plots](Analysis_and_plots) contains R scripts that were used to analyze the data with the R package NBevolution and to plot the results. The scripts can be used to reproduce the analysis as outlined in [Reproduce full analysis](#Reproduce-full-analysis).

## Description of the R package NBevolution

The package [NBevolution](NBevolution_0.0.0.9000.tar.gz) contains custom functions used in the analysis. A brief description of each function is provided in the following, a more detailed explanation is given in the help file of each function. The scripts are tailored towards an analysis using SNV/indel output generated by the DKFZ SNV/indel calling pipeline (https://github.com/DKFZ-ODCF/SNVCallingWorkflow) and CNV estimates generated by ACEseq (https://github.com/DKFZ-ODCF/ACEseqWorkflow).
...
- Driver_mutations.R which summarizes known drivers among the tumors
- Mutation_density_quantification.R which computes the mutation density at each chromosomal segment and at ECA and MRCA of each tumor
- Figure_4.R and Figure_5.R which source / use output generated by the following scripts
- Mutation_density_quantification.R which computes the mutation density at each chromosomal segment and at ECA and MRCA of each tumor
...
- Figure_6.R wich sources / uses output generated by the following scripts
- Survival_analysis.R which analyzes the survival statistics in both cohorts
- Figure_8.R which sources / uses output generated by the following scripts
