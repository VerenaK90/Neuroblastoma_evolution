...
primary.of.tumor.pairs <- c("XI003_10023", "XI003_20195")
## and relapse tumor of tumor pairs
relapse.of.tumor.pairs <- c("XI003_10023_relapse", "XI003_20195_relapse")
...
clonal.mutation.matrix. <- matrix(0, nrow=1 + 2 + 3 + 4 , ncol=length(c(tumors.discovery, tumors.validation)),
                                 dimnames=list(c("monosomic", "disomic_I", "disomic_II", "trisomic_I", "trisomic_II", "trisomic_III",
                                               ...
                                               c(tumors.discovery, tumors.validation)))
...
## One matrix to store only CpG>TpG mutations to later correct densities at ECA/MRCA
clonal.cpg_tpg.mutation.matrix <- clonal.mutation.matrix
...
for(i in c(tumors.discovery, tumors.validation)){
  ...
  ## Correct purity estimate for tumor XI003_19079 manually
  if(i == "XI003_19079"){
    purity.refit <- T
  }else{
    purity.refit <- F
  ...
  clonal.cpg.mutations <- count.clonal.mutations(aceseq, mutations, purity.refit=purity.refit, chromosomes = c(1:22), CpG_TpG = T)
  ...
  for(j in 1:22){
    ...
    clonal.cpg_tpg.mutation.matrix[[j]][,i] <- clonal.cpg.mutations$clonal.mutation.matrix[,j]
...
save(ploidy, purity, copy.number.indicator, B.allele.indicator, segment.length.matrix, clonal.mutation.matrix,
     clonal.cpg_tpg.mutation.matrix,
...
earliest.mutation.time <- earliest.mutation.time[intersect(names(earliest.mutation.time),c(tumors.discovery, tumors.validation))]
earliest.mutation.time.lower <- earliest.mutation.time.lower[intersect(names(earliest.mutation.time),c(tumors.discovery, tumors.validation))]
earliest.mutation.time.upper <- earliest.mutation.time.upper[intersect(names(earliest.mutation.time),c(tumors.discovery, tumors.validation))]
mutation.time.mrca.lower <- mutation.time.mrca.lower[c(tumors.discovery, tumors.validation)]
mutation.time.mrca.upper <- mutation.time.mrca.upper[c(tumors.discovery, tumors.validation)]
mutation.time.mrca <- mutation.time.mrca[c(tumors.discovery, tumors.validation)]
mutation.time.eca <- mutation.time.eca[c(tumors.discovery, tumors.validation)]
mutation.time.eca.lower <- mutation.time.eca.lower[c(tumors.discovery, tumors.validation)]
mutation.time.eca.upper <- mutation.time.eca.upper[c(tumors.discovery, tumors.validation)]
gains.at.earliest.time <- gains.at.earliest.time[intersect(names(gains.at.earliest.time),c(tumors.discovery, tumors.validation))]
gains.at.mrca <- gains.at.mrca[intersect(names(gains.at.mrca),c(tumors.discovery, tumors.validation))]
gains.uniquely.mapped.to.eca <- gains.uniquely.mapped.to.eca[intersect(names(gains.uniquely.mapped.to.eca),c(tumors.discovery, tumors.validation))]
gains.not.maping.to.eca.or.mrca <- gains.not.maping.to.eca.or.mrca[intersect(names(gains.not.maping.to.eca.or.mrca),c(tumors.discovery, tumors.validation))]
monosomic.states.not.matching.mrca <- monosomic.states.not.matching.mrca[intersect(names(monosomic.states.not.matching.mrca),c(tumors.discovery, tumors.validation))]
gains.at.mrca.conforming.eca <- gains.at.mrca.conforming.eca[intersect(names(gains.at.mrca.conforming.eca),c(tumors.discovery, tumors.validation))]
...
for(i in 1:length(c(tumors.discovery, tumors.validation))){
  fragments.timing.mrca <- mutation.time.upper[[c(tumors.discovery, tumors.validation)[i]]][sapply(names(mutation.time.upper[[c(tumors.discovery, tumors.validation)[i]]]), function(x){
...
for(i in c(tumors.discovery, tumors.validation)){
  ...
  if(i %in% tumors.discovery){
    pdf(paste0(data.directory.discovery, i, "/VAF_histograms.pdf"), width=5, height=5)
    print(ggarrange(plotlist = p, nrow=2, ncol=2))
    dev.off()
  }else if(i %in% tumors.validation){
    pdf(paste0(data.directory.validation, i, "/VAF_histograms.pdf"), width=5, height=5)
    print(ggarrange(plotlist = p, nrow=2, ncol=2))
    dev.off()
  }
